...
int main(int argc, char* argv[])
{
  ...
  ROS_WARN("subscribe image topic :%s imu_topic: %s", image_topic.c_str(), imu_topic.c_str());
...
void processBagfile(string bagfile)
{
  ...
  string rgb_tpc = image_topic; // "/cam0/color"; // "/cam0/image_raw";
  // string dpt_tpc = "/cam0/depth";
  // string ir_tpc  = "/cam0/ir";
  // string ir2_tpc = "/cam0/ir2";
  string imu_tpc = imu_topic; // "/imu"; // /imu0
  ...
  BOOST_FOREACH(rosbag::MessageInstance const m, view)
  {
    ...
    }else // an image mssage
    {
      ...
      {
        // receive a dpt image
        cv_ptrD = cv_bridge::toCvShare(simage, sensor_msgs::image_encodings::TYPE_32FC1);
        // imwrite(d_dpt + "/"+tt.str() +".exr", cv_ptrD->image);
